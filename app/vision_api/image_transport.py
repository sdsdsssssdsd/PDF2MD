...
from app.vision_api.errors import NetworkError, RequestTooLargeError, UnsupportedImageError
...
def encode_base64(path: Path) -> ImagePayload:
    if path.suffix.lower() not in SUPPORTED and path.suffix.lower() != ".bmp":
        raise UnsupportedImageError(f"不支持的图片格式: {path.suffix}")
    data = path.read_bytes()
    if len(data) > 40 * 1024 * 1024:
        raise RequestTooLargeError(f"图片过大: {path.name}")
    mime = guess_mime(path)
    b64 = base64.standard_b64encode(data).decode("ascii")
    return ImagePayload(path=path, mime=mime, data_url=f"data:{mime};base64,{b64}")
...
def prepare_images(
    ...
    *,
    ...
    for p in paths:
        p = Path(p)
        if not p.is_file():
            raise UnsupportedImageError(f"图片不存在: {p}")

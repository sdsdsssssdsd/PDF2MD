...
PROMPT_VERSION = "vision-transcribe-v2.2"
...
5. 行内数学公式使用 $...$。第一次输出就必须正确，不要写成 \\(...\\) 或裸 (n)。
- 语境变量如「若 (n) 为奇数」必须写成「若 $n$ 为奇数」
- 公式编号引用如「公式 (19)」「式 (18)」「Eq. (21)」保持原文，不要加 $

6. 独立数学公式必须使用多行 $$ 围栏，禁止单行横排，禁止 \\[...\\]，禁止用单独一行的 [ ... ] 冒充公式：
$$
...
$$
原稿中的行间公式第一次就必须写成上述围栏。
...
9. 公式内容不得纠错、简化、补充或重新推导。但必须修复明显的 LaTeX/OCR 转义损坏，例如：
...
16. 禁止使用 --- 分隔符，禁止在章节之间插入横线。不要输出 YAML front matter。
